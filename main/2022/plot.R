...
# logo images
logo_qatar <- rasterGrob(readPNG("logo_qatar.png"), interpolate=TRUE)
logo_my <- rasterGrob(readPNG("logo.png"), interpolate=TRUE)

# group background coordinates
...
plot <- data %>% 
  ...
  annotation_custom(logo_my, xmin = -27, xmax = 27, ymin = -102, ymax = -84) +
  # flag text
